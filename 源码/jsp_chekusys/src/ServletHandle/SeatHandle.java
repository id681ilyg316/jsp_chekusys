...
import java.util.*;
			...
			private void deleteEntity() throws IOException
			{
				...
				response.sendRedirect("/chekusys/SeatHandle?type=4");//删除成功后跳转至管理页面
			}
...
String seat_tag=new String(request.getParameter("seat_tag").getBytes("ISO8859_1"),"UTF-8");
				...
				{
					try {
						response.sendRedirect("/chekusys/SeatHandle?type=4");//成功更新数据后跳转至SeatMsg.jsp页面
					} catch (IOException e) {
			...
			private void insertEntity() throws UnsupportedEncodingException, IOException
			{
				...
				if(!seat.checkExist(seat_id))
				{
					...
					{
						out.write("<script>alert('数据添加成功！'); location.href = '/chekusys/SeatHandle?type=4';</script>");
					}
					else {
						out.write("<script>alert('数据添失败！'); location.href = '/chekusys/SeatHandle?type=4';</script>");
					}
				}
				else {
					out.write("<script>alert('主键重复，数据添加失败！'); location.href = '/chekusys/SeatHandle?type=4';</script>");
				}
			}
