...
import java.util.*;
			...
			private void deleteEntity() throws IOException
			{
				...
				response.sendRedirect("/chekusys/FixedHandle?type=4");//删除成功后跳转至管理页面
			}
			
			//车辆出场更新操作
			private void setOut() throws IOException
			{
				String card_id=new String(request.getParameter("cardid").getBytes("ISO8859_1"),"UTF-8");
			    ...
			    {
			    	if(parkList.contains(card_id))
			    	  parkList.remove(card_id);
			    	response.sendRedirect("/chekusys/FixedHandle?type=6");
			    }
			}
			...
			private void updateEntity() throws UnsupportedEncodingException
			{
				...
				{
					try {
						response.sendRedirect("/chekusys/FixedHandle?type=4");//成功更新数据后跳转至FixedMsg.jsp页面
					} catch (IOException e) {
			...
			private void insertEntity() throws UnsupportedEncodingException, IOException
			{
				...
				if(parkList.contains(card_id)) {
					out.write("<script>alert('入库失败：该车辆已入库，不可重复入库！'); location.href = '/chekusys/FixedHandle?type=6';</script>");
					return;
				}

				if((!fixed.checkExist(fixed_id))&&(!card_id.equals(-1)))
				{
					if(fixed.insertEntity(fixed_id,card_id,entry_date,entry_time,out_date,out_time)==1)
					{
						parkList.add(card_id);
						out.write("<script>alert('数据添加成功！'); location.href = '/chekusys/FixedHandle?type=6';</script>");
					}
					else {
						out.write("<script>alert('数据添加失败！'); location.href = '/chekusys/FixedHandle?type=6';</script>");
					}
				}
				else {
					out.write("<script>alert('主键重复，数据添加失败！'); location.href = '/chekusys/FixedHandle?type=4';</script>");
				}
			}
