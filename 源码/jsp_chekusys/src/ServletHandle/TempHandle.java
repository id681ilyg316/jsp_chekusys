...
import java.util.*;
			...
			private void deleteEntity() throws IOException
			{
				...
				response.sendRedirect("/chekusys/TempHandle?type=4");//删除成功后跳转至管理页面
			}
...
String temp_money=new String(request.getParameter("temp_money").getBytes("ISO8859_1"),"UTF-8");
				...
				{
					try {
						response.sendRedirect("/chekusys/TempHandle?type=4");//成功更新数据后跳转至TempMsg.jsp页面
					} catch (IOException e) {
			...
			private void insertEntity() throws UnsupportedEncodingException, IOException
			{
				...
				if(!temp.checkExist(card_id))
				{
					...
					{	System.out.println("card_id="+card_id);
						...
						out.write("<script>alert('数据添加成功！'); location.href = '/chekusys/TempHandle?type=4';</script>");
					     }
					else {
						out.write("<script>alert('数据添失败！'); location.href = '/chekusys/TempHandle?type=4';</script>");
					}
					 }
					else{
						out.write("<script>alert('临时IC卡号或者车牌号不能为空'); location.href = '/chekusys/TempHandle?type=4';</script>");
					    }
					
				}
				else {
					out.write("<script>alert('主键重复，数据添加失败！'); location.href = '/chekusys/TempHandle?type=4';</script>");
				}
			}
