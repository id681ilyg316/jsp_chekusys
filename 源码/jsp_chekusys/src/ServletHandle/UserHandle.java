...
import java.util.*;
			...
			private void chagePwd() throws IOException
			{
				...
				if(user.checkPwd(userId, oldPwd))
				{
					...
					{
						out.write("<script>alert('密码更改成功~~~');location.href='/chekusys/Common/UserInfo.jsp'</script>");
					}
					else {
						out.write("<script>alert('密码更改失败~~~');location.href='/chekusys/Common/ChagePwd.jsp'</script>");
					}
				}
				else {
					out.write("<script>alert('原始密码错误~~~');location.href='/chekusys/Common/ChagePwd.jsp'</script>");
				}
			}
			...
			private void register() throws UnsupportedEncodingException, IOException
			{
				...
				if(!user.checkExist(UserId))
				{
					if(user.insertEntity(UserId,RoleId,UserName,RealName,UserPwd,UserPhone)==1)
					{
					    ...
					    out.write("<script>alert('恭喜你，注册成功~'); location.href = '/chekusys/Login.jsp';</script>");
					}
				}
				else {
					  out.write("<script>alert('您注册的登陆账号已存在，请重新注册！'); location.href = '/chekusys/Login.jsp';</script>");
				}
			}
			...
			private void deleteEntity() throws IOException
			{
				...
				response.sendRedirect("/chekusys/UserHandle?type=4");//删除成功后跳转至管理页面
			}
			...
			private void updateEntity() throws UnsupportedEncodingException
			{
				...
				if(user.updateEntity(user_id,role_id,user_name,real_name,user_pwd,user_phone)==1)
				{
					try {
							...
							{
								response.sendRedirect("/chekusys/UserHandle?type=4");//成功更新数据后跳转至UserInfo.jsp页面
							}
							else {
								response.sendRedirect("/chekusys/Common/UserInfo.jsp");//成功更新数据后跳转至UserInfo.jsp页面
							}
					} catch (IOException e) {
			...
			private void insertEntity() throws UnsupportedEncodingException, IOException
			{
				...
				if(!user.checkExist(user_id))
				{
					...
					{
						out.write("<script>alert('数据添加成功！'); location.href = '/chekusys/UserHandle?type=4';</script>");
					}
					else {
						out.write("<script>alert('数据添失败！'); location.href = '/chekusys/UserHandle?type=4';</script>");
					}
				}
				else {
					out.write("<script>alert('主键重复，数据添加失败！'); location.href = '/chekusys/UserHandle?type=4';</script>");
				}
			}
