			...
			private void deleteEntity() throws IOException
			{
				...
				response.sendRedirect("/chekusys/RoleHandle?type=4");//删除成功后跳转至管理页面
			}
...
String role_name=new String(request.getParameter("role_name").getBytes("ISO8859_1"),"UTF-8");
				...
				{
					try {
						response.sendRedirect("/chekusys/RoleHandle?type=4");//成功更新数据后跳转至RoleMsg.jsp页面
					} catch (IOException e) {
...
String role_name=new String(request.getParameter("role_name").getBytes("ISO8859_1"),"UTF-8");
				...
				if(!role.checkExist(role_id))
				{
					...
					{
						out.write("<script>alert('数据添加成功！'); location.href = '/chekusys/RoleHandle?type=4';</script>");
					}
					else {
						out.write("<script>alert('数据添失败！'); location.href = '/chekusys/RoleHandle?type=4';</script>");
					}
				}
				else {
					out.write("<script>alert('主键重复，数据添加失败！'); location.href = '/chekusys/RoleHandle?type=4';</script>");
				}
			}
