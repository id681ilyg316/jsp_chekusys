...
import java.util.*;
			...
			private void deleteEntity() throws IOException
			{
				...
				response.sendRedirect("/chekusys/CardHandle?type=4");//删除成功后跳转至管理页面
			}
			...
			private void updateEntity() throws UnsupportedEncodingException
			{
				...
				{
					try {
						response.sendRedirect("/chekusys/CardHandle?type=4");//成功更新数据后跳转至CardMsg.jsp页面
					} catch (IOException e) {
			...
			private void insertEntity() throws UnsupportedEncodingException, IOException
			{
				...
				if(!card.checkExist(card_id))
				{
					...
					{
						out.write("<script>alert('数据添加成功！'); location.href = '/chekusys/CardHandle?type=4';</script>");
					}
					else {
						out.write("<script>alert('数据添失败！'); location.href = '/chekusys/CardHandle?type=4';</script>");
					}
				}
				else {
					out.write("<script>alert('主键重复，数据添加失败！'); location.href = '/chekusys/CardHandle?type=4';</script>");
				}
			}
