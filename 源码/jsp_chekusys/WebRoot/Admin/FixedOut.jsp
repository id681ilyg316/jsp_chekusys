...
<fieldset>
	<legend>添加入库信息</legend>
	<form action="<%=path %>/FixedHandle?type=3" class="Form">
		<table style=" margin:10px 0;"> 
			...
			<tr><td>IC卡号：</td><td>
				...
				<input type="submit" value="入库" id="btnSure"/>
			</td></tr>
	            </tbody>      
	        </table>
	</form>
				...
				out.write("<input type='hidden' name='hvalue' value='"+(request.getAttribute("value")!=null?request.getAttribute("value").toString():"")+"' />");
                		if(list!=null) //循环数据列表，生成表格行
                		{
                			for(int i=0;i<list.size();i++)
	                		{
	               				...
	               				out.print("<tr><td>"+obj[0]+"</td><td>"+obj[1]+"</td><td>"+obj[7]+"</td><td>"+obj[6]+"</td><td>"+obj[2]+" "+obj[3]+"</td><td>"+result+"</td><td><a href='"+path+"/FixedHandle?cardid="+card_id+"&type=10&fixed_id="+obj[0]+"' class='a_del' onclick='return confirm(\"是否确认出场？\")'>设置出场</a></td></tr>");
	                		}
                		}
		...
		<tfoot>
            		<tr><td colspan="7">
						...
						<%
            					...
            					<span>跳转至</span><input type="text" class="txtPage" /><input type="button" value="跳转" class="btnGo" />
            				</div>
            			</td>
			</tr>
            </tfoot>
